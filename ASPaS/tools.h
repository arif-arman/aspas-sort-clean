...
#ifdef __AVX__
        if (std::is_same<T, int>::value)
            LINE_SIZE = (int)simd_width::AVX_INT;
        if (std::is_same<T, float>::value)
            LINE_SIZE = (int)simd_width::AVX_FLOAT;
        if (std::is_same<T, double>::value)
            LINE_SIZE = (double)simd_width::AVX_DOUBLE;
#endif
#ifdef __MIC__
        if (std::is_same<T, int>::value)
            LINE_SIZE = (int)simd_width::AVX512_INT;
        if (std::is_same<T, float>::value)
            LINE_SIZE = (int)simd_width::AVX512_FLOAT;
        if (std::is_same<T, double>::value)
            LINE_SIZE = (double)simd_width::AVX512_DOUBLE;
#endif
        for (i = 0; i < size; i++) {
            std::cerr << arr[i] << "\t";
            if ((i + 1) % LINE_SIZE == 0)
